...
char bb_ins[7][5] = {
		"ble", "bge", "blt", "bgt", "bne", "beq"
};

int is_bb(int ins) {
	return ins >= 37 && ins <= 42;
}

char btf_literal[3][10] = {
		"ifFlase",
		"ifTrue"
};
...
int is_btf(int ins) {
	return ins == INS_BNEZ || ins == INS_BEQZ;
}

char binary_op_ins[17][5] = {
		"",
		"add",//INS_ADD = 1,
		"sub", //INS_SUB = 2,
		"mul", //INS_MUL = 3,
...
char unary_op_literal[3][5] = {
		"~",
		"-"
};

char unary_op_ins[3][5] = {
		"not",
		"neg"
};
...
struct Block {//保证每个基本块以一个label开头，0-1条件跳转与一个无条件跳转结尾
	int id, in_deg; //基本块的编号；基本块的入度，优化用
	int ins_size, buffer_size; //基本块的IR指令vector
	Instruction *ins; //大多地方都是用单链表，因为顺序不敏感，但是指令不一样
...
struct Array {
	int num, mul; //当前维度的宽度；后面维度的宽度乘积
	Array *nxt, *pre; //维度的前进，后退，其实后退只要在初始化的时候用到
};
...
struct Identifier {
	char *id; //id的名字
	int level; //如果是一个变量，只针的重数
	int from; //如果这个变量是一个复合类型member，则记录其偏移量，如union均为0
	int is_var; //如果是变量为1，如果是typedef的declarator为0
	int arg_num; //如果是函数的参数，为从右数起1-base的编号
	Array *array; //数组的状态
	Type *type; //类型
	Identifier *nxt; //自带链表
	int init_type; //初始化的类型，用列表、用字符串、不初始化
	char *init_str; //初始化的字符串的字面值，用于输出到.data段
	char *str_val; //字符串的值
	InitPair *init_list; //用于初始化的list，在输出到.data段的.word .byte时候用
	Environment *env_belong; //属于哪一层环境，在生成代码的时候栈内存和全局变量要分开判断
	Type *type_belong; //属于哪一个struct or union
} *o_buffer;

struct Type { //在parse一个type specifier的时候产生
	int is_struct; //struct or union
	int width; //支持类型的预申明不定义，此时类型宽度为-1，在定义完了之后依照%4补齐方便挪动
	char *_literal; //类型名
	Type *nxt; //自带链表
	Identifier *mem; //成员的链表
} *type_int, *type_char, *type_void; //预定义三个基础类型

struct Environment { //环境符号表
	Environment *pre; //上一层环境
	Type *types; //这层环境里面定义的类型链表
	Identifier *ids; //这层环境里面定义的id链表
} *global, *cur_env; //全局层；当前层
...
int is_branch(int);
int is_binary(int);
Identifier *find_str(char *);

int is_binary(int ins) {
	return ins >= 1 && ins <= 16;
}

int is_unary(int ins) {
	return ins == INS_NOT || ins == INS_NEG;
}
...
ReturnType *array_write(ReturnType *l, ReturnType *r) {
	...
	append_ins(block_top, ins_cons(INS_ARRAY_WRITE, 0, l, r));
	return r;
}
...
ReturnType *binary_instruction(int op, ReturnType *l, ReturnType *r) {
	if (l->const_val && r->const_val) {
		if (op == INS_ADD) {
			return make_const_ReturnType(l->const_val + r->const_val);
		} else if (op == INS_SUB) {
			return make_const_ReturnType(l->const_val - r->const_val);
		} else if (op == INS_MUL) {
			return make_const_ReturnType(l->const_val * r->const_val);
...
ReturnType *parse_arguments(Function *func) {
	...
	if (!my_strcmp(func->id, "__print_int__")) {
		...
		if (arg_cnt) {
			if (format->ref->id[0] == '$') {
				//printf("# const output format\n");
				Identifier *id = find_str(format->ref->id);
				if (!my_strcmp(id->str_val, "%d")) {
					append_ins(block_top, ins_cons(INS_PRINT_INT, 0, nd_buffer[0], 0));
					return 0;
				}
				if (!my_strcmp(id->str_val, "%d ")) {
					append_ins(block_top, ins_cons(INS_PRINT_INT, 0, nd_buffer[0], 0));
					append_ins(block_top, ins_cons(INS_PUTCHAR, 0, make_const_ReturnType(' '), 0));
					return 0;
				}
				if (!my_strcmp(id->str_val, "%d\n")) {
					append_ins(block_top, ins_cons(INS_PRINT_INT, 0, nd_buffer[0], 0));
					append_ins(block_top, ins_cons(INS_PUTCHAR, 0, make_const_ReturnType('\n'), 0));
					return 0;
				}
				if (!my_strcmp(id->str_val, "%c")) {
					append_ins(block_top, ins_cons(INS_PUTCHAR, 0, nd_buffer[0], 0));
					return 0;
				}
			}
...
void parse_program() {
	...
	while (cur_scan < src_tail) {
		if (my_strcmp(look._literal, "typedef")) {
			...
			Type *type = parse_type_specifier(def);
			decl = parse_declarators();
//			if (!my_strcmp(decl->_literal, "Declarator")) {
//				printf("%d\n", (int) find_type(global->types, type->_literal));
//				printf("add it T_T\n");
//			}
			while (decl) {
				add_var_to_env(Identifier_(type, 0, 0, decl));
...
//			printf("find? %d\n", find_id(global->ids, "Declarator"));
			my_assert(!my_strcmp(look._literal, ";"), "everything end with ;");
			look = next_token();
		}
		cur_func = 0;
		//printf("%d\n", (int) find_type(global->types, "Declarator"));
		//printf("%d\n", (int) find_id(global->ids, "Declarator"));
	}
	parsing = 0;
}
...
void initialize() {
	...
	int i;
	for (i = 0; i < 10; ++i) {
		regs[i]._literal = str_cat("$t", to_string(i));
	}
	for (i = 10; i < 18; ++i) {
		regs[i]._literal = str_cat("$s", to_string(i - 10));
	}
	for (i = 18; i < 21; ++i) {
		regs[i]._literal = str_cat("$a", to_string(i - 17));
	}
...
int find_next_use(ReturnType *tar, Block *blck, int from) {
	int i;
	for (i = from; i < blck->ins_size; ++i) {
		if (blck->ins[i].des == tar) {
			return i;
		}
		if (blck->ins[i].a == tar) {
			return i;
...
void show_blck(Block *blck) {
	...
	for (i = 0; i < blck->ins_size; ++i) {
		ReturnType *des = blck->ins[i].des;
		ReturnType *a = blck->ins[i].a;
		ReturnType *b = blck->ins[i].b;
		if (des) {
			des->reg = 0;
		}
		if (a) {
...
void show_Instruction(Instruction *ins) {
	if (is_unary(ins->ins)) {
		int op = ins->ins == INS_NEG;
		printf("# %s = %s%s\n", ReturnType_to_string(ins->des), unary_op_literal[op], ReturnType_to_string(ins->a));
		IR2ins_load_value("$t1", ins->a);
		printf("\t %s $t0, $t1\n", unary_op_ins[op]);
		IR2ins_store_back(ins->des, "$t0");
	} else if (INS_LD_ADDR == ins->ins) {
		printf("# %s = &%s\n", ReturnType_to_string(ins->des), ReturnType_to_string(ins->a));
		IR2ins_load_addr("$t1", ins->a);
		IR2ins_store_back(ins->des, "$t1");
	} else if (is_binary(ins->ins)) {
		printf("# %s = %s %s %s\n", ReturnType_to_string(ins->des), ReturnType_to_string(ins->a), binary_op_literal[ins->ins],
			   ReturnType_to_string(ins->b));
		if (ins->a->ret_type == CONST_VAL) {
			IR2ins_load_value("$t1", ins->a);
			IR2ins_load_value("$t2", ins->b);
			printf("\t%s $t0, $t1, $t2\n", binary_op_ins[ins->ins]);
		} else if (ins->b->ret_type == CONST_VAL) {
			IR2ins_load_value("$t1", ins->a);
			IR2ins_load_value("$t2", ins->b);
			printf("\t%s $t0, $t1, $t2\n", binary_op_ins[ins->ins]);
		} else {
		...
		IR2ins_store_back(ins->des, "$t0");
	} else if (is_btf(ins->ins)) {
		int op = ins->ins == INS_BNEZ;
		printf("# %s %s goto \n", btf_literal[op], ReturnType_to_string(ins->a));
		IR2ins_load_value("$t0", ins->a);
		printf("\t%s $t0, ", btf_ins[op]);
	...
	} else if (is_bb(ins->ins)) {
		int op = ins->ins - 37;
		printf("# if %s %s %s goto\n", ReturnType_to_string(ins->a), bb_literal[op], ReturnType_to_string(ins->b));
		IR2ins_load_value("$t1", ins->a);
		IR2ins_load_value("$t2", ins->b);
		printf("\t%s $t1, $t2, ", bb_ins[op]);
	} else {
		printf("%d\n", ins->ins);
