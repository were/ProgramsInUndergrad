...
void Grid::output_picture(int photon_number) {
	...
	for (int i = 0; i < points.size(); ++i) {
		...
		pic_real[point.h][point.w][0] += point.flux.x * (1.0 / (M_PI * point.r2 * photon_number));
		pic_real[point.h][point.w][1] += point.flux.y * (1.0 / (M_PI * point.r2 * photon_number));
		pic_real[point.h][point.w][2] += point.flux.z * (1.0 / (M_PI * point.r2 * photon_number));
	}
	for (int i = 0; i < HEIGHT; ++i)
		for (int j = 0; j < WIDTH; ++j) {
			if (i==414 && j==28800) {
				pic_unsigned_char[i][j][0] = 255;
				pic_unsigned_char[i][j][1] = 0;
				pic_unsigned_char[i][j][2] = 0;
			}
			else {
				for (int k = 0; k < 3; ++k)
					pic_unsigned_char[i][j][k] = to_unsigned_char(pic_real[i][j][k]);
			}
		}
