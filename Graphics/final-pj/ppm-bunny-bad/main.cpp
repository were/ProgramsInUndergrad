...
//		fprintf(stderr, "%d %d %.5f %.5f %.5f\n",l,depths[l],rays[l].pos.x,rays[l].pos.y,rays[l].pos.z);
//		if (l)
//			fprintf(stderr, "%.5f %.5f %.5f \n",rays[l].pos.x,rays[l].pos.y,rays[l].pos.z);
		if (depths[l] > 20)
			continue;
...
int main() {
	...
	for (int h = 0; h < HEIGHT; ++h) {
		...
		for (int w = 0; w < WIDTH; ++w) {
			pixel_h = h;
			pixel_w = w;
			real x = (1.0 * w - WIDTH / 2) / WIDTH;
			real z = (-1.0 * h + HEIGHT / 2) / WIDTH;
