...
void Grid::output_picture(int photon_number, int epochs_number, real global_r2) {
	...
	static double pre[HEIGHT][WIDTH][3];
	...
	for (int i = 0; i < HEIGHT; ++i)
		for (int j = 0; j < WIDTH; ++j) {
			for (int k = 0; k < 3; ++k) {
				...
				if (photon_number == 10000000) {
					double cur = (pic_real[i][j][k] + old_real[i][j][k]) / epochs_number;
					res += (pre[i][j][k] - cur) * (pre[i][j][k] - cur);
					pre[i][j][k] = cur;
				}
			}
		}
	...
	if (photon_number == 10000000) {
		char file_name[30];
		sprintf(file_name, "delta_%d", epochs_number);
